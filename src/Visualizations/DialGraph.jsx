...
class DialGraph extends Component {
    ...
    render() {
        ...
        // CURRENT VALUE
        var valueLable = { x: (width/2), y: ((height/2)+12), value: data};

        // SCALES -- not sure if this is better than using the axis method...
        var axisMin = { x: 0, y: height-5, value: dataMin}; // display min scale
        var axisMax = { x: width, y: height-5, value: dataMax}; // display max scale
            ...
            <Card> 
                <CardBody>
                    <svg ref={node => this.node = node} width={this.props.size[0]} height={this.props.size[1]} className='dial-chart'>
                        ...
                        <text className="axisText" x={axisMax.x} y={axisMax.y} text-anchor="end">{axisMax.value}</text>
